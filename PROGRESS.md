...
> 最后更新: 2026-07-23 (v11.3 文档同步与仓库卫生修复)
...
| 项目 | 目录 | 说明 |
|---|---|---|
| 聚丙烯供应链监测 | `daily_pp_monitor/` | 五指标信号跟踪（现货价 / 库存天数 / 基差 / 开工率 / 霍尔木兹通航），七层传导框架的实战应用，2026-07-16 建立基线 |
...
| 版本标注漂移 | 🟢 已解决 | README/INDEX/CONTINUE_FOR_AI 曾停留在 v10.0，v11.1 备份文件内部错标 v11.0 | ✅ v11.3 修复 |
| LITE 版滞后 | 🟢 已解决 | PSYCHOHISTORY_LITE.md 曾停留在 v9.0 方法（8 补丁旧流程） | ✅ v11.3 重生成 |
| case-002 资源依赖 | 🟢 已解决 | HTML 案例依赖被 gitignore 的 `cases/_shared/`，克隆后无法渲染 | ✅ v11.3 改 CDN |
| daily_pp_monitor 无文档 | 🟢 已解决 | 监测目录未登记，数据断更 | ✅ v11.3 登记 |
...
| 版本 | 日期 | 变更 |
|------|------|------|
| **v11.3** | 2026-07-23 | 文档同步与仓库卫生修复：五处版本标注（README/INDEX/CONTINUE_FOR_AI/PROGRESS/LITE）同步至 v11.2 口径；PSYCHOHISTORY_LITE.md 重生成至 v11.2；系统提示词历史版本归档至 `versions/`；case-002 共享资源改 CDN 加载；`daily_pp_monitor/` 登记入文档；分析报告 PDF 入库至 `cases/`；补登 v11.0–v11.2 版本记录 |
| **v11.2** | 2026-07-15 | 系统提示词两层规则重构（方案 B）：8 个强制补丁 → 必须层（3 条）+ 增强层（3 条条件触发）；§3.5 轮换表降为可选增强；§5 弹性密度（当时未同步文档，v11.3 补登） |
| **v11.1** | 2026-07-15 | 系统提示词新增技能解释与密度梯度补丁（当时未同步文档，v11.3 补登） |
| **v11.0** | 2026-07-15 | 系统提示词反平庸补丁；`methodology/convergence-verify.md` §6 新增冷门系列轮询规则（当时未同步文档，v11.3 补登） |
