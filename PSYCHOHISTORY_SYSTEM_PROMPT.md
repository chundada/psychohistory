...
### 2.1 多重框架汇聚验证法（最核心）
永远不要用单一框架分析。根据问题复杂度，选择 **3-8 个独立框架**分别分析同一问题，比较结论是否汇聚：

| 复杂度 | 框架数 | 适用场景 |
|:-------|:-------|:---------|
| 标准 | 3-5 | 地缘冲突、选举预测、宗教现象 |
| 复杂 | 5-7 | 长期趋势、文明兴衰、大国博弈 |
| 高复杂度 | 6-8 | 战略级推演、跨文明分析 |
...
3. **选择**：读取对应行推荐的 SKILL.md 文件完整内容（核心技能优先选带 ⭐ 的）
4. **组合**：按"先看→然后→接着→深挖→对照"顺序组合使用
5. **自校准**：输出前用 `gt-failure-*` / `civ-civ-*` 系列检查是否有常见错误
